...
@test "'..' navigates up one directory" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/a' && .. && pwd"
    [[ "${status}" -eq 0 ]]
...
@test "'...' navigates up two directories" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/a/b' && ... && pwd"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == "${BATS_TEST_TMPDIR}" ]]
}
...
@test "'....' navigates up three directories" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/a/b/c' && .... && pwd"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == "${BATS_TEST_TMPDIR}" ]]
}
...
@test "'cp' copies a file to a new destination" {
    ...
    local dst="${BATS_TEST_TMPDIR}/dst.txt"
    echo "hello" > "${src}"
    cp "${src}" "${dst}"
    [[ -f "${dst}" ]]
    [[ "$(cat "${dst}")" == "hello" ]]
...
@test "'cp' copies a directory recursively" {
    mkdir -p "${BATS_TEST_TMPDIR}/srcdir/subdir"
    echo "hello" > "${BATS_TEST_TMPDIR}/srcdir/file.txt"
    echo "world" > "${BATS_TEST_TMPDIR}/srcdir/subdir/nested.txt"
    _run_alias "cp '${BATS_TEST_TMPDIR}/srcdir' '${BATS_TEST_TMPDIR}/dstdir'"
    [[ -f "${BATS_TEST_TMPDIR}/dstdir/file.txt" ]]
    [[ -f "${BATS_TEST_TMPDIR}/dstdir/subdir/nested.txt" ]]
}
...
@test "'ga' stages a file for commit" {
    _setup_git_repo "${BATS_TEST_TMPDIR}/repo"
    cd "${BATS_TEST_TMPDIR}/repo"
    echo "content" > file.txt
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && ga file.txt"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" status --short
    [[ "${output}" == *"A  file.txt"* ]]
}

@test "'gbr' lists local branches" {
    _setup_git_repo "${BATS_TEST_TMPDIR}/repo"
    git -C "${BATS_TEST_TMPDIR}/repo" commit --allow-empty -m "init"
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gbr"
    [[ "${status}" -eq 0 ]]
    [[ -n "${output}" ]]
}
...
@test "'gbrd' deletes a local branch" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gbrd to-delete"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" branch
    [[ "${output}" != *"to-delete"* ]]
}

@test "'gbrdd' deletes a branch locally and from the remote" {
    local fake_git="${BATS_TEST_TMPDIR}/bin/git"
...
@test "'gc' creates a commit" {
    ...
    echo "content" > file.txt
    git add file.txt
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gc -m 'test commit'"
    [[ "${status}" -eq 0 ]]
...
@test "'gca' amends the most recent commit" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gca --allow-empty -m 'amended message'"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" log --oneline
...
@test "'gch' checks out a branch" {
    ...
    git commit --allow-empty -m "init"
    git branch feature
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gch feature"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" rev-parse --abbrev-ref HEAD
    [[ "${output}" == "feature" ]]
}
...
@test "'gcl' clones a repository" {
    ...
    _run_alias "gcl '${origin}' '${BATS_TEST_TMPDIR}/clone'"
    [[ -d "${BATS_TEST_TMPDIR}/clone/.git" ]]
}
...
@test "'gclf' clones only the latest snapshot of a repository" {
    ...
    _run_alias "gclf 'file://${origin}' '${BATS_TEST_TMPDIR}/shallow'"
    run git -C "${BATS_TEST_TMPDIR}/shallow" log --oneline
    [[ "$(echo "${output}" | wc -l)" -eq 1 ]]
}
...
@test "'gdf' shows the diff of working-tree changes" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gdf"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *"-original"* ]]
    [[ "${output}" == *"+changed"* ]]
}
...
@test "'gdfc' shows the diff of staged changes" {
    ...
    echo "changed" > file.txt
    git add file.txt
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gdfc"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *"-original"* ]]
    [[ "${output}" == *"+changed"* ]]
}
...
@test "'gl' shows the commit log in one-line format" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gl"
    [[ "${status}" -eq 0 ]]
...
@test "'gre' unstages a staged file" {
    ...
    git add file.txt
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gre HEAD file.txt"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" status --short
    [[ "${output}" == *"??"* ]]
}
...
@test "'grem' lists remote repositories" {
    _setup_git_repo "${BATS_TEST_TMPDIR}/repo"
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && grem"
    [[ "${status}" -eq 0 ]]
}
...
@test "'grm' removes a tracked file from the index" {
    ...
    git add file.txt
    git commit -m "init"
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && grm file.txt"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" status --short
    [[ "${output}" == *"D  file.txt"* ]]
}
...
@test "'gs' shows the working-tree status in short format" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gs"
    [[ "${status}" -eq 0 ]]
...
@test "'gt' creates an annotated tag" {
    ...
    cd "${BATS_TEST_TMPDIR}/repo"
    git commit --allow-empty -m "init"
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gt v1.0.0 -m 'version 1.0.0'"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" tag --list
    [[ "${output}" == *"v1.0.0"* ]]
}
...
@test "'gtl' lists all tags" {
    ...
    git tag v0.1.0
    git tag v0.2.0
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gtl"
    [[ "${output}" == *"v0.1.0"* ]]
    [[ "${output}" == *"v0.2.0"* ]]
}
...
@test "'gz' stashes working-tree changes" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gz"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" status --short
    [[ "${status}" -eq 0 ]]
    [[ -z "${output}" ]]
...
@test "'gza' applies a stash without removing it" {
    ...
    echo "modified" > file.txt
    git stash
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gza"
    [[ "${status}" -eq 0 ]]
    [[ "$(cat "${BATS_TEST_TMPDIR}/repo/file.txt")" == "modified" ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" stash list
    [[ "${output}" == *"stash@{0}"* ]]
}
...
@test "'gzc' stashes changes but keeps staged files in the index" {
    ...
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gzc"
    [[ "${status}" -eq 0 ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" status --short
    [[ "${output}" == *"staged.txt"* ]]
    [[ ! -f "${BATS_TEST_TMPDIR}/repo/unstaged.txt" ]]
}
...
@test "'gzl' lists stash entries" {
    ...
    echo "modified" > file.txt
    git stash
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gzl"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *"stash@{0}"* ]]
}
...
@test "'gzp' pops a stash entry and restores changes" {
    ...
    echo "modified" > file.txt
    git stash
    _run_alias "cd '${BATS_TEST_TMPDIR}/repo' && gzp"
    [[ "${status}" -eq 0 ]]
    [[ "$(cat "${BATS_TEST_TMPDIR}/repo/file.txt")" == "modified" ]]
    run git -C "${BATS_TEST_TMPDIR}/repo" stash list
    [[ -z "${output}" ]]
...
@test "'lns' creates a symbolic link using absolute paths" {
    local src="${BATS_TEST_TMPDIR}/source_file"
    local lnk="${BATS_TEST_TMPDIR}/link_file"
...
@test "'ll' lists files with detailed info including hidden files" {
    touch "${BATS_TEST_TMPDIR}/.hidden"
    _run_alias "ll '${BATS_TEST_TMPDIR}'"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *".hidden"* ]]
    [[ "${output}" == *"total"* ]]
}
...
@test "'la' lists hidden files" {
    touch "${BATS_TEST_TMPDIR}/.hidden"
    _run_alias "la '${BATS_TEST_TMPDIR}'"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *".hidden"* ]]
}

@test "'l' lists files" {
    touch "${BATS_TEST_TMPDIR}/visible.txt"
    _run_alias "l '${BATS_TEST_TMPDIR}'"
    [[ "${status}" -eq 0 ]]
    [[ "${output}" == *"visible.txt"* ]]
}
...
@test "'mkd' creates a directory and navigates into it" {
    cd "${BATS_TEST_TMPDIR}"
    FA__mkd "new_dir"
    [[ "$(pwd)" == "${BATS_TEST_TMPDIR}/new_dir" ]]
    [[ -d "${BATS_TEST_TMPDIR}/new_dir" ]]
}

@test "'mkd' creates nested directories" {
    cd "${BATS_TEST_TMPDIR}"
    FA__mkd "a/b/c"
    [[ "$(pwd)" == "${BATS_TEST_TMPDIR}/a/b/c" ]]
...
@test "'FA__mvnt' runs 'mvn test' without arguments" {
    local fake_mvn="${BATS_TEST_TMPDIR}/bin/mvn"
    mkdir -p "${BATS_TEST_TMPDIR}/bin"
    cat > "${fake_mvn}" <<'EOF'
#!/usr/bin/env bash
echo "mvn $*"
...
@test "'rm' removes a file after confirmation" {
    local file="${BATS_TEST_TMPDIR}/to_delete.txt"
    echo "content" > "${file}"
...
@test "'uvp' runs a poe task via uv" {
    local fake_uv="${BATS_TEST_TMPDIR}/bin/uv"
    mkdir -p "${BATS_TEST_TMPDIR}/bin"
    cat > "${fake_uv}" <<'EOF'
#!/usr/bin/env bash
echo "uv $*"
EOF
    chmod +x "${fake_uv}"
    _run_alias "PATH='${BATS_TEST_TMPDIR}/bin:${PATH}' uvp my-task"
    [[ "${status}" -eq 0 ]]
